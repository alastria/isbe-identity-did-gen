...
const didCLI = new DidCommands(
  provider,
  signerToUse,
  RPC_URL,
  ellipticTypeFromConfig
);
...
program
  ...
  .action(async (ellipticType) => {
    const alreadyInitialized = await isRegistryInitialized(
      provider,
...
program
  ...
  .action(
    async (privKey: string, baseDocumentPos: string | undefined, opts: any) => {
      try {
  ...
  .action(async (privKey: string, baseDocument: string, options: any) => {
    try {
      ...
      try {
        ...
        console.error(
          chalk.red(
            "El baseDocument no es un JSON válido. Pásalo entre comillas."
          )
        );
        return;
      }
       const did = await didCLI.createChild(
        normalized,
        baseDocument,
        options.aka
      );
      ...
      console.error(
        chalk.red("Error en createSecondary:"),
        err.message || err
      );
    }
  });
...
program
  ...
  .action(async (opts) => {
    try {
      ...
      console.error(
        chalk.red("Error en get-did-by-timestamp-onchain:"),
        e.message || e
      );
    }
  });
...
program
  ...
  .action(async (opts) => {
    await didCLI.getDidByTimestamp(opts.did, Number(opts.timestamp));
  });
...
program
  ...
  .action(async (opts) => {
    await controllerCmd.listDidsByController(
      opts.controller,
      Number(opts.page),
...
program
  ...
  .requiredOption(
    "--name <string>",
    "Nombre de la relación (authentication, assertionMethod, etc.)"
  )
...
program
  ...
  .requiredOption(
    "--name <string>",
    "Nombre del verification relationship"
  )
  .option("--page <num>", "Página", "1")
  .option("--pageSize <num>", "Tamaño de página", "10")
  .action(async (opts) => {
